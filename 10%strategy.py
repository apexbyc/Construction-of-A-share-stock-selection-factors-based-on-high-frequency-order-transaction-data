...
AFTERNOON_START_INCLUSIVE = time(13, 0)  # 包含下午开始时间 13:00:00
...
def calculate_threshold_top10_percent(s: pd.Series) -> float:
    """
    计算阈值：取前10%分位数
    即90%的数据小于这个值，10%的数据大于等于这个值
    """
    if len(s) < 2:
        return s.max() if len(s) else 0.0
    # 注意：这里使用0.9表示90%分位数，即前10%
    threshold = s.quantile(0.9)

    # 如果分位数计算失败（如全为0），返回最大值
...
def one_day_stock_factor(secucode: str, date: str, df: pd.DataFrame) -> Optional[dict]:
    ...
    # 3.3 阈值计算
    buy_big_thr = calculate_threshold_top10_percent(buy["buy_volume"])
    sell_big_thr = calculate_threshold_top10_percent(sell["sell_volume"])
    buy_long_thr = calculate_threshold_top10_percent(buy["buy_duration"])
    sell_long_thr = calculate_threshold_top10_percent(sell["sell_duration"])
   ...
   # print(f"   分组准备耗时: {timings['分组准备']:.2f}s ，任务数: {len(groups)}")
    ...
    if results:
        ...
        if output_dir:
            ...
            out_path = os.path.join(output_dir, "高频订单因子_前10阈值.feather")
            factors_df.reset_index(drop=True).to_feather(out_path)

            csv_path = os.path.join(output_dir, "高频订单因子_前10阈值.csv")
